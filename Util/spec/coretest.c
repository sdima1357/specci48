...
uint16_t sw(uint16_t inp)
{
	uint16_t val = inp;
	//~ uint8_t *it = (uint8_t *) &inp;
	//~ uint8_t *ot = (uint8_t *) &val;
	//~ ot[0] = it[1];
	//~ ot[1] = it[0];
	return val;
}
uint16_t swap1(uint16_t inp)
{
	uint16_t val = inp;
	uint8_t *it = (uint8_t *) &inp;
	uint8_t *ot = (uint8_t *) &val;
...
read_test( FILE *f, libspectrum_dword *end_tstates )
{
  ...
  F  =((u8*)& af)[0];
  A  =((u8*)& af)[1];
  BC  = sw(bc);  DE  = sw(de);  HL  = sw(hl);
  AF_ = (af_); BC_ = sw(bc_); DE_ = sw(de_); HL_ = sw(hl_);
  ...
  if( fscanf( f, "%x %x %u %u %u %d %d", &i, &r, &iff1, &iff2, &im,
	      &thalt, &end_tstates2 ) != 7 ) {
    fprintf( stderr, "%s: second registers line in `%s' corrupt\n", progname,
	     testsfile );
    return 1;
  }

  I = i; R = R7 = r; IFF1 = iff1; IFF2 = iff2; IM = im;halt = thalt;
...
dump_z80_state( void )
{
  ...
  printf( "%04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x\n",
	  tt, BC, DE, HL, (AF_), BC_, DE_, HL_, IX, IY, SP, PC, MEMPTR );
  printf( "%02x %02x %d %d %d %d %d\n", I, ( R7 & 0x80 ) | ( R & 0x7f ),
	  IFF1, IFF2, IM, halt, tstates );
}
